...
class ProductTemplate(models.Model):
    ...
    def _prepare_price_per_quantity(
        self, product, price, price_per_quantity_uom
    ):
        ...
        uom_categ_type = self._uom_lookup_by_categ().get(
            price_per_quantity_uom.category_id)
    ...
    def _calc_list_price_per_quantity(self):
        for product in self:
            ...
            if vals:
                product.list_price_per_quantity = (
                    self._calc_price_per_uom(**vals))
            else:
...
class ProductProduct(models.Model):
    ...
    lst_price_per_quantity = fields.Monetary(
        string="Price per quantity",
        compute="_calc_lst_price_per_quantity",
    )
    ...
    def _calc_lst_price_per_quantity(self):
        ...
        for product in self:
            ...
            if vals:
                product.lst_price_per_quantity = (
                    tmpl._calc_price_per_uom(**vals)
                )
            else:
